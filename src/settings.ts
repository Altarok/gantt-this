...
export type FantasyGanttSettings = {
  defaultType: string
...
export class FantasyGanttSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    this.addCalenderSelection(containerEl);
    this.addColorSeletion(containerEl);
  ...
  private addColorSeletion(containerEl: HTMLElement) {
  ...
  private addCalenderSelection(containerEl: HTMLElement) {
  ...
  private renderMapSettings(
    ...
    .addText(text => {
      ...
      if (parentSetting) {
        const picker = parentSetting.createEl('input', {
          attr: {
            type: 'color',
            value: defaultColor,
            style: 'margin: 0 10px;'
